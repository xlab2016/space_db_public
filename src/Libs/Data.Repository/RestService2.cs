...
using Data.Repository.Models.Rest;
...
{
    public abstract class RestService2<T, TKey, TDto, TQuery, TMap>
        ...
        where TDto : class
    {
        ...
        protected RestService2(ILogger<RestServiceBase<T, TKey>> logger, IDapperDbContext restDapperDb, DbContext restDb, string tableName,
            TMap map)
        {
            this.logger = logger;
        ...
        protected virtual async Task<PagedList<TDto>> SearchUsingEfAsync<TFilter, TSort>(QueryBase<T, TFilter, TSort> query,
            Func<IQueryable<T>, IQueryable<T>> expression, MapOptions mapOptions = null, IQueryable<T> restSource = null, Action<PagedList<T>> onPagedList = null)
            where TFilter : FilterBase<T>
            where TSort : SortBase<T>
        {
            return await RetryHelper.RetryDbAsAsync(async () =>
            {
                var list = await FilterHelper.SearchUsingEfAsync(query, expression, restDb, restSource);
        ...
        protected virtual async Task<TDto> FindUsingEfAsync(TKey key,
            Func<IQueryable<T>, IQueryable<T>> expression, Func<T, T> postFunc = null, MapOptions mapOptions = null, IQueryable<T> restSource = null)
        {
            return await RetryHelper.RetryDbAsAsync(async () =>
            {
        ...
        public virtual async Task<object> UpdateAsync(TDto request)
        {
            ...
            {
                return await restSet.FirstOrDefaultAsync(_ => _.Id.Equals(source.Id));
            });
